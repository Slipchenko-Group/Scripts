...
def pd2arr(df, mol_name, device, label_col, n_steps, n_atoms):
    # shahed: i have edited this function to return a force tensor as well
    # this was on 3/8/23, need to have force_x force_y and force_z in the datset
    ...
    for a in range(0, n_atoms):
        ...
        forces_xyz_arr = mol_df[['force_x', 'force_y', 'force_z']].to_numpy()[a::n_atoms]
        forces_xyz_arr = np.expand_dims(forces_xyz_arr, axis=0).reshape(n_steps, 1, 3)
        force_arrs.append(forces_xyz_arr)
    ...
    force_tensor = torch.tensor(np.concatenate(force_arrs, axis=1))
    return(coord_tensor, energy_tensor, elecpot_tensor, molid_arrs, force_tensor)
