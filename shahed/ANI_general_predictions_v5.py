...
# version 5
# this version will incorporate real qm/efp interaction from calculations into the final csv
# need to have a gas .out files to be places in the directory in their own ./gas folder
# messed with what the true energy was and other stuff
# added a few lines to check for if the gas or efp efp jobs were errors, did not do this previously and cause garbage data generation
# corrected the true interaction energy

data = [fl for fl in os.listdir(os.getcwd()) if fl.endswith('qmefp.in.out') or fl.endswith('qmefp.out')]

# data_dict = {'predicted energy': [], 'true energy': [],'total system energy':[], 'qm energy':[], 'solvent/efp energy': [], 'predicted interaction energy': [], 'interaction2': [], 'mol_name': [],'system energy no efp': [], 'system energy': [], 'gas energy': [],'true interaction energy':[] }
data_dict = {'mol_name': [],'true qm energy': [],'true qm energy no efp': [], 'true gas energy': [],'true interaction energy': [], 'ANI predicted qm energy': [], 'ANI predicted interaction energy': [], 'ANI gas energy': [], 'ANI solvent/efp energy' : [], 'ANI total system energy': [], 'ANI interaction2':[]}
    ...
    with open(d,'r') as fl:
        ...
        for idx, ln in enumerate(out_lns):
            ...
            if 'Total energy in the final basis set' in ln:
                ...
                system_energy_no_efp = float(ln.split()[-1])
                os.chdir('./efp')
                with open(f'./{mol_name}_qmefp.in.out', 'r') as efpfl:
                    efp_out_lns = efpfl.readlines()
                    for ln2 in efp_out_lns:
                        if 'TOTAL EFP ENERGY' in ln2:
                            system_energy_no_efp -= float(ln2.split()[-1])
                            break
                os.chdir('..')
                os.chdir('./gas')
                if 'mobley' in mol_name.split('_')[0]:
                    with open(f'./{mol_name}_gas.in.out') as gasfl:
                        gas_out_lns = gasfl.readlines()
                        for ln3 in gas_out_lns:
                            if 'Total energy in the final basis set' in ln3:
                                gas_energy = float(ln3.split()[-1])
                                break
                else:
                    with open(f'./{mol_name}_gas.out') as gasfl:
                        gas_out_lns = gasfl.readlines()
                        for ln3 in gas_out_lns:
                            if 'Total energy in the final basis set' in ln3:
                                gas_energy = float(ln3.split()[-1])
                                break
                os.chdir('..')
                true_interaction = system_energy_no_efp - gas_energy
        ...
        # data_dict['system energy no efp'].append(system_energy_no_efp)
        # data_dict['system energy'].append(true_energy)
        # data_dict['gas energy'].append(gas_energy)
        # data_dict['true interaction energy'].append(true_energy - gas_energy)
        ...
        # check for gas or efp efp jobs being errors and append all at once
        if 'Error: in the serial run' in efp_out_lns[-2] or 'Error: in the serial run' in efp_out_lns[-1]:
            lol = 1
        else:
            if 'Error: in the serial run' in gas_out_lns[-2] or 'Error: in the serial run' in gas_out_lns[-1]:
                lol2 = 1
            else:
                data_dict['ANI predicted qm energy'].append(energies)
                data_dict['mol_name'].append(mol_name)
                data_dict['true qm energy no efp'].append(system_energy_no_efp)
                data_dict['true qm energy'].append(true_energy)
                data_dict['true gas energy'].append(gas_energy)
                data_dict['true interaction energy'].append(true_interaction)
                data_dict['ANI predicted interaction energy'].append(interaction_energy)
                data_dict['ANI gas energy'].append(qm_energy)
                data_dict['ANI solvent/efp energy'].append(efp_energy)
                data_dict['ANI total system energy'].append(total_energy)
                data_dict['ANI interaction2'].append(int2)


df = pd.DataFrame.from_dict(data_dict)
df.to_csv(r'ANI_general_predictions_data_v5_4.csv', index = False, header = True)
